...
#since we have 9 sprites( for each direction) lets run it at 27 FPS so 3FPS per sprite
...
class player(object):
    def __init__(self,x,y,width,height):
        ...
        self.walkCount = 0
        self.standing = True
        self.hitbox = (self.x +17, self.y + 11 , 29 , 52)#4 things in a tuple is a rectangle
    ...
    def hit(self):
        ...
        text = font1.render ("Score: -5", 1, (255,0,0))
        win.blit(text, (250 - (text.get_width()/2), 200))#we draw the text, x coordinate (center) and y coordinate = 200
...
#main loop
font = pygame.font.SysFont("comicsans", 30, True, True)#(name,size,bold,italic)
...
while run:
    ...
    if not (man.isJump):
        #this will happen if the user is not jumping
        if keys[pygame.K_UP]:
            man.isJump = True
            man.right = False
            man.left = False
        ...
        #this will happen if the user is jumping.
        if man.jumpCount >= -10:
            neg =1 
            if man.jumpCount < 0:
                neg = -1
            #during the jump, y is going to go up 500 pixels and then decrease 1 by 1
            man.y -= (man.jumpCount**2) * 0.5 * neg
            man.jumpCount -= 1

        else:
            #jump has finished, isJump = False, reset jumpCount
            man.isJump = False
            man.jumpCount = 10   
